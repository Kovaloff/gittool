...
class GitRepo
{
    ...
    public function test_git()
    {
        ...
        $pipes = array();
        $resource = proc_open(GitTool::get_bin(), $descriptorspec, $pipes);
    ...
    {
        return $this->run_command(GitTool::get_bin() . " " . $command);
    }
     ...
     * @return string
     */
    public function log($format = null, $fulldiff = false, $filepath = null, $follow = false, $maxcount = 50)
    {
        ...
        if ($format === null) {
            ...
            return $this->run('log --decorate --max-count='.$maxcount.' --pretty=format:"' . $format . '" ' . $diff . $filepath);
        }
    }
    ...
    {
        return $this->run("remote show origin");
    }

    /**
     * Resets Repo to latest Commit.
     *
     * @return string
     */
    public function repo_reset()
    {
        return $this->run("reset --hard HEAD");
    }
